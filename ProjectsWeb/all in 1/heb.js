...
function payById(){	
	...
	if(paymentId !== ""){
		...
		id:$('#form4 form input[name="paymentId"]').val()
			...
			if(response=="False"){alert("לא נמצא תשלום מתאים למזהה זה.");}
			else{
				var form = $(response);
				$('body').append(form);
				form.submit();
		...
		alert("אנא הקלד מספר מזהה תשלום");
	}
}
function payByForm(){	
	...
	if(amount !== ""&&firstName !== ""&&lastName !== ""&&night_phone_a !== ""){
		...
		amount:amount	
			...
			if(response.indexOf("False")==0){alert("נוצרה שגיאה: "+response);}
			else{
				var form = $(response);
				$('body').append(form);
				form.submit();
		...
		alert("אנא הכנס את סכום התשלום, שם פרטי, שם משפחה ומספר טלפון ליצירת קשר");
	}
}