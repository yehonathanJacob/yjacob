...
path_to_root = os.environ[
				   'yjacob_private_root'] + "\\PythonLab\\JJFunction"  # Path to root where you have your private files
...
def getHebDate(day="", month="", year="", city="Tel+Aviv"):
	...
	for value in a['items']:
		s += "\t\t{Hebrew}|{Title}\t\t|{Category}\t\n".format(Hebrew=value['hebrew'][::-1], Title=value['title'],
															  Category=value['category'])
	s += '(' + dNow.strftime('%A') + ', ' + dNow.strftime('%B') + ' ' + dNow.strftime('%d') + ' ' + dNow.strftime(
		'%Y') + ')'
	return s
...
class updateBackUp(object):
	...
	def setSrcAndDest(self, Src='', Dest=''):
		...
		if Dest != '':
			...
			self.root_dest = filedialog.askdirectory(initialdir="/",
													 title="Please select a destination folder to copy to")
	...
	def startCp(self):
		if self.root_dest == '' or self.root_src == '':
			...
			raise
		if not (os.path.exists(self.root_src) and os.path.isdir(self.root_src)) or not (
				os.path.exists(self.root_dest) and os.path.isdir(self.root_dest)):
...
FARR = [show, setInString, printDirectoris, moovPointerOfFile, getDir, getPath, getMyPath, BinToDec, HexToDec, DecToBin,
		ShowFileInEnding, getHebDate, changeSongs, openAssets]
