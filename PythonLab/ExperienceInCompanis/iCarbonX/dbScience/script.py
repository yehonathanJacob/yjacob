...
def get_usda_refs_Sheet1():
	conn = pyodbc.connect(
		r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=C:\Users\janjak2411\Desktop\PythonLab\icarbonx\dbScience\WorkArea\WorkArea.accdb;')
	cursor = conn.cursor()
...
def addDataToOutput(arr):
	...
	for obj in arr:
		cursor.execute(
			"INSERT INTO [Output] ( FoodDB_name, usda_name, usda_id, langual_tags, langual_descriptions)  VALUES ('%s','%s','%s','%s','%s');" % (
			obj['FoodDB_name'], obj['usda_name'], obj['usda_id'], obj['langual_tags'], obj['langual_descriptions']))
	conn.commit()
	conn.close()
