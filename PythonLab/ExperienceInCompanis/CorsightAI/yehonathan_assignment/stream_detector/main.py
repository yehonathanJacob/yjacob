...
PREFETCH_COUNT = 1  # Backpressure control
...
async def process_frame_message(message: IncomingMessage) -> None:
    ...
    Decodes frame data, performs face detection, and sends results to
    downstream service.
    ...
    Raises:
        Exception: Re-raises exceptions to trigger message requeue/DLQ
    """
    async with message.process():
        try:
            ...
            logger.info(
                f"Detected {len(detected_faces)} faces in frame {frame_index}"
            )
...
async def start_consumer() -> None:
    ...
    try:
        ...
        await rabbitmq_channel.set_qos(prefetch_count=PREFETCH_COUNT)
