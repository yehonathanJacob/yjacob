...
def step2_makeResepisList(a,errors=[],resepis=[]):
	...
	for link in a:
		try:
			...
			resList =  response.css(".tasty-recipe-ingredients ul li::text").extract()

			title = response.selector.css('title::text').extract()[0]
			title = title.replace(" - Cookie and Kate","")
			img = response.selector.css('.content .entry-content p img::attr(data-lazy-src)').extract()
			if len(img)==0:
				img = ""
			else:
				img = img[0]
...
def getData():
	print("path:cookieandkate\\recipesUp2.json")
	f = codecs.open("cookieandkate\\recipesUp2.json", "a+", "utf-8")
	f.seek(0,0)
	old  = f.read()
	f.close()
	print("len: %d"%(len(old)))
